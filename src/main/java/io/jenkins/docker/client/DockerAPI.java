...
public class DockerAPI extends AbstractDescribableImpl<DockerAPI> {
    ...
    public DockerAPI(DockerServerEndpoint dockerHost, String registryCredentialsId) {
        this.dockerHost = dockerHost;
    ...
    private static SharableDockerClient makeClient(
        ...
        try {
            ...
            if (registryCredentialsId != null) {
                DockerRegistryCredentials creds = getRegistryCredentials(registryCredentialsId);
                DockerClientConfig config = DefaultDockerClientConfig.createDefaultConfigBuilder()
                    .withRegistryUsername(creds.getUsername())
                    .withRegistryPassword(creds.getPassword().getPlainText())
                    .withRegistryEmail(creds.getEmail())
                    .withRegistryUrl(creds.getRegistryUrl())
                    .build();
    ...
    private static DockerRegistryCredentials getRegistryCredentials(String credentialsId) {
        return firstOrNull(
            lookupCredentials(DockerRegistryCredentials.class, Jenkins.get(), ACL.SYSTEM, List.of()),
            withId(credentialsId));
    }
